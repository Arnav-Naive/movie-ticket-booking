...
function WebSmashIntro({ children }) {
  ...
  const radials = useMemo(() => Array.from({ length: 10 }, (_, i) => {
    const angle = (i / 10) * Math.PI * 2;
    return {
      x1: center.x, y1: center.y,
      x2: center.x + Math.cos(angle) * 260,
      y2: center.y + Math.sin(angle) * 260,
    };
  }), []);
  const rings = [50, 95, 145, 200];

  // Locked once — never recalculated on re-render
  ...
  const shardKeyframes = useMemo(() => shards.map(s => `
    @keyframes fly-${s.id} {
      to { transform: translate(${s.tx}px, ${s.ty}px) rotate(${s.rot}deg); opacity: 0; }
    }
  `).join(''), [shards]);
    ...
    <div style={{ position: 'relative', minHeight: '70vh', overflow: 'hidden' }}>
      ...
      {phase !== 'done' && (
        <div style={{
          ...
          {phase === 'impact' && (
            <div style={{ position: 'absolute', inset: 0, background: '#fff', animation: 'flash 0.15s ease-out forwards' }} />
          )}
          ...
          {(phase === 'web' || phase === 'impact') && (
            <svg viewBox="0 0 400 300" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
              ...
              {rings.map((r, i) => (
                <circle key={`c-${i}`} cx={center.x} cy={center.y} r={r}
          ...
          <span style={{
            ...
            letterSpacing: '4px', opacity: phase === 'web' ? 0 : 1, transition: 'opacity 0.2s',
          ...
          {phase === 'shatter' && shards.map(s => (
            <div key={s.id} style={{
              ...
              clipPath: s.clip,
              animation: `fly-${s.id} 0.6s cubic-bezier(.5,0,.9,.4) ${s.delay}s forwards`,
              opacity: 0.95,
            }} />
          ))}
        </div>
